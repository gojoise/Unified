...
import { SCAN_SOURCE_LABELS, type ScanConfidence, type ScanEntry } from '../types/scan.types'
...
export default {
  setup() {
    ...
    const progressPercent = computed(() =>
      scan.progress.total > 0 ? (scan.progress.scanned / scan.progress.total) * 100 : 0,
    )
    ...
    const exeName = (exePath: string) => exePath.split(/[\\/]/).pop() ?? exePath

    /** Chemin de l'exécutable relatif au dossier d'installation, plus lisible. */
    const relativeExe = (entry: ScanEntry) => {
      const suffix = entry.exePath.slice(entry.installDir.length).replace(/^[\\/]/, '')
      return entry.exePath.startsWith(entry.installDir) ? suffix : entry.exePath
    }
